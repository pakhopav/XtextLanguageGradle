...
class GrammarKitGenerator(val targetPath: String, val languageExtension: String, val context: MetaContext) {


    constructor(builder: XtextModuleBuilder) : this(
        builder.contentEntryPath!!,
    ...
    protected fun updatePluginXml(project: Project) {
        val pluginXmlFilePath = "$targetPath/src/main/resources/META-INF/plugin.xml"
        val virtualFile =
    ...
    protected fun generateParser(module: Module) {
        ...
        val bnfGrammarFilePath =
            "$targetPath/src/main/java/${extension}Language/${extension}/grammar/${languageExtension.capitalize()}.bnf"
    ...
    protected fun generateLexer(module: Module) {
        ...
        val pairs: List<Pair<VirtualFile, DownloadableFileDescription>>? =
            service.createDownloader(descriptions, libraryName)
                .downloadWithProgress(targetPath, project, null)
        assertNotNull(pairs)
        ...
        val extension = languageExtension.toLowerCase()
        val flexFilePath =
            "${targetPath}/src/main/java/${extension}Language/${extension}/grammar/${extension.capitalize()}.flex"
