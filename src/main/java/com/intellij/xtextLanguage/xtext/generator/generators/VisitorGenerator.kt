...
class VisitorGenerator(extension: String, private val model: VisitorGeneratorModel) : AbstractGenerator(extension) {


    fun generateNameVisitor() {
        val path = File(myGenDir + "/psi")
        val file = File(myGenDir + "/psi/${extension.capitalize()}NameVisitor.kt")
        path.mkdirs()
        file.createNewFile()
        ...
        out.print("""
            ...
            |class ${extension.capitalize()}NameVisitor {
        """.trimMargin("|"))
        out.print("\n")
        model.rules.forEach {
            val rule = it
            val methodName = "visit${it.name}"
            out.print("    fun $methodName (node : ${extension.capitalize()}${it.name}): PsiElement?{\n")
            it.uniqueName?.let {
